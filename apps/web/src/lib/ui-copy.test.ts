...
describe("ui copy preferences", () => {
  ...
  it("falls back to system when browser storage is unavailable", () => {
    ...
    expect(readUiLanguagePreference(storage, "kodeks.ui.language")).toBe(
      "system",
    );
  ...
  it("re-localizes welcome and framework errors when the UI language changes", () => {
    expect(
      localizeAssistantMessageContent(uiCopy.zh.app.welcome, uiCopy.en),
    ).toBe(uiCopy.en.app.welcome);
    expect(
      localizeAssistantMessageContent(uiCopy.en.app.welcome, uiCopy.zh),
    ).toBe(uiCopy.zh.app.welcome);
    expect(
      localizeAssistantMessageContent(uiCopy.zh.app.requestFailed, uiCopy.en),
    ).toBe(uiCopy.en.app.requestFailed);
    expect(
      localizeAssistantMessageContent(
        "Runtime failed: socket closed",
        uiCopy.zh,
      ),
    ).toBe("运行失败：socket closed");
    expect(
      localizeAssistantMessageContent("运行失败：连接已断开", uiCopy.en),
    ).toBe("Runtime failed: 连接已断开");
  });
