...
import { ShellCommandTimeoutError, runApprovedCommand } from "@kodeks/workspace";
  ...
  try {
    if (decision === "reject") {
      const approval = await database.approvals.reject(
        id,
        typeof body.reason === "string" && body.reason.trim() ? body.reason.trim() : "Rejected by user"
      );
    ...
    if (command === null) {
      return Response.json({ error: "Approval does not contain an executable command." }, { status: 400 });
    }

    const approved = await database.approvals.approve(id);
    const result = await runApprovedCommand(command, { cwd: getKodeksWorkspace().rootPath() });
