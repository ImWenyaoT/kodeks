...
- `packages/model`: DeepSeek Chat Completions adapter + OpenAI Responses API fallback，共用同一个 runtime provider contract。
...
PRD 的产品目标仍然是 memory、multi-session、plan mode、subagent、workspace tools、shell execution 和 auditable approvals；变化只是实现栈从 Python/FastAPI 切到 TypeScript/Next.js/Bun，并把模型层抽成 Chat Completions 与 Responses API 都能兼容的 adapter。
...
- Official API/product reference: TypeScript 主链路优先支持 DeepSeek Chat Completions；OpenAI Responses API 保留为 fallback。`packages/model` 负责把 streaming、function tools、tool result items 翻译成 kodeks 自己的 provider/runtime contract。
...
换句话说：模型 API surface 不能绑定单一 provider，而要按 Kodeks 内部 `ModelClient` contract 落地；当前 DeepSeek Chat Completions 是默认路径，OpenAI Responses API 是 fallback；agent 产品形态、状态边界、调度策略优先研究 `/Users/edward/Documents/src`；`opencode` 用来做第二视角的结构对照，不能反过来压过 `/src`。
...
| --- | --- | --- | --- | --- |
| Model streaming / function tools | DeepSeek Chat Completions + OpenAI Responses API docs | `services/api/*` outbound adapter 分层 | `packages/llm/src/protocols/*`, `packages/opencode/src/provider/*` | 用 TypeScript model adapter 翻译 text delta、tool_call、tool_result continuation，route/runtime 不直接依赖 provider 细节。 |
...
- `packages/model`: outbound model provider adapters，目前默认 DeepSeek Chat Completions，并保留 OpenAI Responses API fallback。
