...
- `packages/model`: OpenAI Responses API adapter。
...
- Official API/product reference: TypeScript 主链路使用 OpenAI Responses API。`packages/model` 负责把 streaming、function tools、tool result items 翻译成 kodeks 自己的 provider/runtime contract。
- Legacy provider reference: `legacy/python` 保留 DeepSeek/OpenAI-compatible Chat Completions 实现，用来做历史行为对照；它不是当前主应用的 model API surface。
...
换句话说：API surface 按 OpenAI Responses API 落地；agent 产品形态、状态边界、调度策略优先研究 `/Users/edward/Documents/src`；`opencode` 用来做第二视角的结构对照，不能反过来压过 `/src`。
...
| --- | --- | --- | --- | --- |
| Responses streaming / function tools | OpenAI Responses API docs | `services/api/*` outbound adapter 分层 | `packages/llm/src/protocols/*`, `packages/opencode/src/provider/*` | 用 TypeScript model adapter 翻译 response stream、function_call、function_call_output，route/runtime 不直接依赖 SDK 细节。 |
...
| Multi-session / resume | Responses API input items and optional response ids | `utils/sessionStorage.ts`, `utils/listSessionsImpl.ts`, `remote/RemoteSessionManager.ts` | `packages/opencode/src/session/session.ts`, `packages/opencode/src/v2/session.ts`, `packages/opencode/src/share/session.ts` | `session_id` 不只是 latest response id，还要逐步支持 transcript、metadata、resume、project/worktree scope。 |
...
- `packages/model`: outbound model provider adapter，目前是 OpenAI Responses API。
...
| Multi-session / Context | Phase 4 | 已实现 conversation state + 最小 transcript | 用 user/assistant transcript 组装 Responses API input items。 |
...
所有 Phase 7 之后的设计都要先做一次 `/Users/edward/Documents/src` 对照阅读，再用 `/Users/edward/Documents/opencode` 做结构交叉检查；涉及 model API、streaming、function tools 时，同时对照 OpenAI Responses API 官方文档。不要从零发明一套看起来像 agent、但没有成熟产品边界的抽象。

## Phase 5A Acceptance Criteria

- [x] provider request 可以携带 `read_file` tool definition。
- [x] Responses API function_call 能被翻译成 `tool_call` runtime event。
...
- `function_call_output`: runtime 把工具结果交回 Responses API provider，让模型继续生成最终答案。
