...
- Official API/product reference: 使用 OpenAI Python SDK 和 OpenAI Responses API。实现前优先对照官方文档；`services/api/openai_responses.py` 负责把 Responses streaming、`previous_response_id`、function tools、`function_call_output` 翻译成 kodeks 自己的 provider/runtime contract。
- Responses starter app reference: 参考 `/Users/edward/Documents/openai-responses-starter-app`，重点看 Responses streaming、tool/function calling、MCP/connectors、tool-call 展示和 session demo。它是 OpenAI API 使用样例，不是 agent runtime 架构主参考。
- Agent design primary reference: 优先参考 `/Users/edward/Documents/src` 的 coding-agent 设计，把成熟产品里的 context window、memory、multi-session、plan mode、tool orchestration、subagent 思路翻译成 Python/FastAPI/OpenAI SDK 实现。
...
换句话说：API surface 按 OpenAI 官方和 starter app；agent 产品形态、状态边界、调度策略优先研究 `/Users/edward/Documents/src`；`opencode` 用来做第二视角的结构对照，不能反过来压过 `/src`。
...
| --- | --- | --- | --- | --- |
| Responses streaming / function tools | `openai-responses-starter-app/app/api/turn_response/route.ts`, `lib/tools/tools.ts` | `services/api/*` outbound adapter 分层 | `packages/llm/src/protocols/*`, `packages/opencode/src/provider/*` | 用 Python adapter 翻译 Responses stream、function tools、`function_call_output`，route/runtime 不直接依赖 SDK 细节。 |
...
| Multi-session / resume | `openai-responses-starter-app/lib/session.ts` 只作为 demo session 对照 | `utils/sessionStorage.ts`, `utils/listSessionsImpl.ts`, `remote/RemoteSessionManager.ts` | `packages/opencode/src/session/session.ts`, `packages/opencode/src/v2/session.ts`, `packages/opencode/src/share/session.ts` | `session_id` 不只是 previous response id，还要逐步支持 transcript、metadata、resume、project/worktree scope。 |
...
| Tool orchestration | `openai-responses-starter-app/lib/assistant.ts`, `lib/tools/tools-handling.ts` 展示 function output 回灌 | `services/tools/toolOrchestration.ts`, `services/tools/toolExecution.ts`, `Tool.ts` | `packages/opencode/src/tool/tool.ts`, `packages/llm/src/tool-runtime.ts`, `packages/llm/src/protocols/utils/tool-stream.ts` | runtime 负责 tool loop；只读工具可并发，高风险/会改状态的工具串行并走 permission/approval。 |
...
- `multi-session resume`: 除 `session_id -> previous_response_id` 外，SQLite session store 现在保存最小 transcript，支持后续 resume UI/API 扩展。
...
| Multi-session / Context | Phase 4 | 已实现 conversation state + 最小 transcript | 用 `session_id -> previous_response_id` 支撑连续上下文，并保存 user/assistant transcript 作为 resume/API 的状态基础。 |
...
所有 Phase 7 之后的设计都要先做一次 `/Users/edward/Documents/src` 对照阅读，再用 `/Users/edward/Documents/opencode` 做结构交叉检查；涉及 Responses API、streaming、function tools、MCP/connectors 时，同时对照 `/Users/edward/Documents/openai-responses-starter-app` 和 OpenAI 官方文档。最后落成 kodeks 的 Python 版本，不要从零发明一套看起来像 agent、但没有成熟产品边界的抽象。

## Phase 5A Acceptance Criteria

- [x] provider request 可以携带 `read_file` tool definition。
- [x] OpenAI function call 能被翻译成 `tool_call` runtime event。
...
- [x] runtime 能把 `function_call_output` 交回 provider。
...
- [x] runtime 仍然只负责编排：收 tool_call、执行 registry、发 tool_result、回传 `function_call_output`；route 和 provider 不执行本地工具。
