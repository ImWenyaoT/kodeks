...
- Official API/product reference: 使用 OpenAI-compatible Python SDK 和 DeepSeek chat-completions API。实现前优先对照 DeepSeek 官方文档；`services/api/deepseek_chat_completions.py` 负责把 streaming、multi-round messages、function tools、tool result messages 翻译成 kodeks 自己的 provider/runtime contract。
- Chat-completions provider reference: DeepSeek 当前是 stateless multi-round messages，不依赖 Responses API 的 `previous_response_id`；runtime/session 必须显式保存 transcript 并组装 messages。
- Agent design primary reference: 优先参考 `/Users/edward/Documents/src` 的 coding-agent 设计，把成熟产品里的 context window、memory、multi-session、plan mode、tool orchestration、subagent 思路翻译成 Python/FastAPI/OpenAI-compatible SDK 实现。
...
换句话说：API surface 按 DeepSeek chat-completions 官方文档；agent 产品形态、状态边界、调度策略优先研究 `/Users/edward/Documents/src`；`opencode` 用来做第二视角的结构对照，不能反过来压过 `/src`。
...
| --- | --- | --- | --- | --- |
| Chat-completions streaming / function tools | DeepSeek chat-completions streaming and function calling docs | `services/api/*` outbound adapter 分层 | `packages/llm/src/protocols/*`, `packages/opencode/src/provider/*` | 用 Python adapter 翻译 chat-completions stream、function tools、tool messages，route/runtime 不直接依赖 SDK 细节。 |
...
| Multi-session / resume | DeepSeek multi-round chat docs define explicit message history | `utils/sessionStorage.ts`, `utils/listSessionsImpl.ts`, `remote/RemoteSessionManager.ts` | `packages/opencode/src/session/session.ts`, `packages/opencode/src/v2/session.ts`, `packages/opencode/src/share/session.ts` | `session_id` 不只是 latest completion id，还要逐步支持 transcript、metadata、resume、project/worktree scope。 |
...
| Tool orchestration | DeepSeek function calling docs define assistant `tool_calls` and `role="tool"` result messages | `services/tools/toolOrchestration.ts`, `services/tools/toolExecution.ts`, `Tool.ts` | `packages/opencode/src/tool/tool.ts`, `packages/llm/src/tool-runtime.ts`, `packages/llm/src/protocols/utils/tool-stream.ts` | runtime 负责 tool loop；只读工具可并发，高风险/会改状态的工具串行并走 permission/approval。 |
...
- `services/api`: outbound model provider adapter，目前是 DeepSeek chat completions。
...
| Multi-session / Context | Phase 4 | 已实现 conversation state + 最小 transcript | 用 user/assistant transcript 组装 chat-completions messages，并保留 latest completion id 作为兼容状态。 |
...
所有 Phase 7 之后的设计都要先做一次 `/Users/edward/Documents/src` 对照阅读，再用 `/Users/edward/Documents/opencode` 做结构交叉检查；涉及 model API、streaming、function tools 时，同时对照 DeepSeek 官方文档。最后落成 kodeks 的 Python 版本，不要从零发明一套看起来像 agent、但没有成熟产品边界的抽象。

## Phase 5A Acceptance Criteria

- [x] provider request 可以携带 `read_file` tool definition。
- [x] DeepSeek function call 能被翻译成 `tool_call` runtime event。
...
- `tool message`: runtime 把工具结果交回 chat-completions provider，让模型继续生成最终答案。
