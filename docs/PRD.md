...
- `src/kodeks/tools.py`: deterministic tool registry for workspace, shell, memory, and explore subagent tools。
- `src/kodeks/workspace.py`: workspace path policy, file service, and shell harness。
- `src/kodeks/storage.py`: SQLite repositories for sessions, messages, memories, approvals, subagent runs, and audit log。
- `src/kodeks/bridge.py`: Responses-to-Chat-Completions bridge，负责 OpenAI Responses DTO、Core IR、Chat Completions adapter。
...
- Official API/product reference: Python 主链路优先使用 OpenAI Agents SDK + Responses API。MoonBridge 这类 Responses-compatible provider 也走 Python Agents SDK runner；DeepSeek/Qwen Chat Completions 作为 MoonBridge upstream，不再暴露成第二套 runtime。
...
换句话说：agent/runtime 不能停在自研雏形；能用 OpenAI Agents SDK 和 Responses API 的地方优先用它们。Kodeks 自己保留 product boundary：workspace policy、session/memory、approval audit、本地 tools、MCP/skills 配置和事件协议。`src/kodeks/config.py` 只负责把 `openai` 与 `moonbridge` 两类配置解析成 Responses-shaped client options。
...
- `src/kodeks/config.py`: Responses-shaped model option resolution；DeepSeek/Qwen Chat Completions 通过 MoonBridge 作为 upstream 接入，不作为第二套 runtime 暴露。
- `src/kodeks/workspace.py`、`src/kodeks/tools.py`、`src/kodeks/storage.py`: workspace/shell 本地能力、tool registry 和 durable state。
