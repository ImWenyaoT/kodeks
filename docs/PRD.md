...
| 协议集成 | 内部使用 Responses-shaped runtime contract，对 OpenAI-compatible Chat Completions 走 MoonBridge；DeepSeek 是默认 upstream。 | bridge adapter tests、model routing tests、tool-call replay tests。 |
...
- `src/kodeks/providers/bridge.py`: Responses-to-Chat-Completions bridge，负责 Chat Completions stream、tool call chunks、DeepSeek `reasoning_content` 扩展和 terminal failure mapping。
...
- MoonBridge 把 OpenAI-compatible Chat Completions stream 转成 Responses-shaped event stream。
  DeepSeek 是默认 upstream，并带有 `reasoning_content` 扩展。
