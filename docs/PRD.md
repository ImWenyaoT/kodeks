...
- `apps/web`: Next.js App Router UI、单一 SSE chat route、sessions API、approvals API。
...
- `packages/model`: OpenAI Responses 与 MoonBridge model option resolution；不再拥有主 agent loop。
...
- `packages/responses-bridge`: 内置 TypeScript Responses-to-Chat-Completions bridge，负责 OpenAI Responses DTO、Core IR、Chat Completions adapter 和本地 HTTP 入口。

PRD 的产品目标仍然是 memory、multi-session、plan mode、subagent、workspace tools、shell execution 和 auditable approvals；变化只是实现栈从 Python/FastAPI 切到 TypeScript/Next.js/pnpm，并明确运行时优先级：OpenAI Agents SDK + Responses API 能覆盖的能力先用它，非 Responses provider 统一先经过 MoonBridge 变成 Responses-shaped runtime。
...
- Official API/product reference: TypeScript 主链路优先使用 OpenAI Agents SDK + Responses API。MoonBridge 这类 Responses-compatible provider 也走 Agents SDK runner；DeepSeek/Qwen Chat Completions 作为 MoonBridge upstream，不再暴露成第二套 runtime。
...
换句话说：agent/runtime 不能停在自研雏形；能用 OpenAI Agents SDK 和 Responses API 的地方优先用它们。Kodeks 自己保留 product boundary：workspace policy、session/memory、approval audit、本地 tools、MCP/skills 配置和事件协议。`packages/model` 只负责把 `openai` 与 `moonbridge` 两类配置解析成 Responses-shaped client options。
...
| -------------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Model streaming / function tools | OpenAI Agents SDK + Responses API docs；DeepSeek/Qwen Chat Completions 经 MoonBridge 适配 | `services/api/*` outbound adapter 分层                                                                    | `packages/llm/src/protocols/*`, `packages/opencode/src/provider/*`                                                                                                            | runtime 先消费 Agents SDK stream events，再映射成 Kodeks `AgentEvent`；MoonBridge 负责把非 Responses upstream 统一成 Responses-compatible endpoint。 |
...
- `packages/model`: Responses-shaped model option resolution；DeepSeek/Qwen Chat Completions 通过 MoonBridge 作为 upstream 接入，不作为第二套 runtime 暴露。
