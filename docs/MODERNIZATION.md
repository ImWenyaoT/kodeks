...
| Auth and safety    | API keys in env/user config, workspace path policy, shell approvals                      | `src/kodeks/config.py`, `workspace.py`, `storage.py`                       | No app-user auth is introduced. Keep provider secrets out of `/api/models`, block workspace escapes/internal paths, keep TS-compatible shell argv parsing, and keep dangerous shell commands behind approval plus audit logs.      | Any command parser behavior change needs a characterization test before cutover.                                                              |
| Configuration      | `~/.kodeks/config.json`, `KODEKS_CONFIG_PATH`, `KODEKS_CONFIG_DIR`, env override         | `src/kodeks/config.py`                                                     | User-level config remains the source of truth. Env values keep final precedence. Model refs stay `provider/model`; MoonBridge remains an implicit adapter for `chat-completions` providers.                                        | Deprecated env aliases must fail with migration guidance instead of silently changing behavior.                                               |
...
| removed `packages/responses-bridge` | `src/kodeks/bridge.py`                                      | Bridge stream parity must pass before Python can handle Chat-Completions-backed models.                             |
| removed `packages/storage`          | `src/kodeks/storage.py`                                     | Schema version and stored wire shapes must remain readable by Python runtime and existing local databases.          |
| removed `packages/workspace`        | `src/kodeks/workspace.py`                                   | Path blocking and shell approval decisions must match current policy.                                               |
| removed `packages/tools`            | `src/kodeks/tools.py` plus Python handlers                  | Tool definitions, result statuses, approval behavior, and read-only filtering must match before agent-loop cutover. |
...
5. Cut the Python agent loop over by default.
   Connect `openai-agents` to Python tools, storage, memory, and model routing. Preserve `AgentEvent` SSE and UI transport mappings. Keep `KODEKS_DIRECT_RESPONSES_RUNTIME=true` as a Python-only adapter diagnostic path, not a TypeScript fallback. Validate deterministic agent loop tests, focused OpenAI/MoonBridge adapter tests, lint, type-check, and route tests that require Python.
...
- Python currently implements the static UI plus FastAPI compatibility routes for health, models, sessions, workspace files, approvals, bridge preflight, MoonBridge `/v1/responses` plus `/responses`, bridge health/model aliases, `/api/chat/stream`, and `/api/chat/ui`. Chat routes use the Python `openai-agents` path by default; the direct Responses adapter is only available through `KODEKS_DIRECT_RESPONSES_RUNTIME=true` for diagnostics.
...
- Python parity tests cover config resolution, secret-free model catalog, provider/model refs, implicit MoonBridge routing, route parity, bridge translation, terminal `response.failed`, direct Responses `error` events, DeepSeek `reasoning_content`, SQLite schema shape and file-DB WAL/busy-timeout setup, parallel file-DB writer connections, workspace path blocking, shell argv parsing, tool schemas, explicit Responses `strict: false` tool defaults, OpenAI hosted tool filtering for MoonBridge, plan-mode read-only filtering, plan artifact creation/recovery, selected workspace file context injection, pre-model memory recall/context injection, large tool output offload to memory artifacts, stateful Responses `previous_response_id`, same-turn tool continuation loops, unknown-tool local halts, multi-turn tool continuation transcript rows, Responses input replay for persisted assistant `output_text`, tool calls, and tool outputs, shell approvals, memory/subagent MVP tools, runtime SSE, UI transport mapping, default Python `openai-agents` agent construction, dynamic `FunctionTool` schema wrapping, strict schema opt-in, SDK approval metadata, Agents SDK input replay, SDK stream event readers, the direct Responses diagnostic branch, FastAPI `/api/chat/stream` plus `/api/chat/ui` routing through the default Agents SDK branch, and smoke-check failure reporting for unreachable live providers.
