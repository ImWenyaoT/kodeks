...
| --- | --- | --- | --- |
| Memory | The agent can store facts, recall them, and offload large tool output into artifacts. | `src/kodeks/storage/memory.py` | `src/kodeks/tools.py`, `src/kodeks/runtime_context.py`, `evals/cases.jsonl` |
...
| Subagent | The current MVP records a small explore-agent run and returns an auditable summary. | `src/kodeks/storage/memory.py` | `src/kodeks/tools.py`, `docs/concepts-map.md` |
...
| Tools | `src/kodeks/tools.py`, `src/kodeks/tool_schemas.py`, `src/kodeks/workspace.py` | Define what the model may call and enforce local workspace/shell boundaries. |
...
| Providers | `src/kodeks/config.py`, `src/kodeks/bridge.py`, `src/kodeks/responses_adapter.py` | Resolve model configuration and keep MoonBridge as an implicit protocol adapter. |
...
2. Read `src/kodeks/tool_schemas.py`, then skim handlers in `src/kodeks/tools.py`.
