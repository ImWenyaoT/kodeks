...
describe("ToolRegistry", () => {
  ...
  it("executes read_file and write_file through workspace boundaries", async () => {
    ...
    const blockedResult = await registry.execute("read_file", {
      path: ".git/config",
    });
  ...
  it("greps visible workspace files and ignores blocked internals", async () => {
    ...
    expect(JSON.parse(result.output)).toMatchObject({
      ok: true,
      matches: [
        { path: "src/a.ts", line: 1, text: "export const marker = 'kodeks';" },
      ],
    });
  });
  ...
  it("turns dangerous shell commands into approval records", async () => {
    ...
    await expect(
      database.approvals.getApproval(output.approvalId),
  ...
  it("stores and recalls memory facts", async () => {
    ...
    expect(JSON.parse(rememberResult.output)).toMatchObject({
      ok: true,
      scope: "project",
    });
    ...
    expect(JSON.parse(recallResult.output).layered.atoms).toEqual([
      expect.objectContaining({
        content: "Kodeks plan mode is read only.",
      }),
    ]);
  ...
  it("reads offloaded memory artifacts by ref id", async () => {
    const registry = buildDefaultToolRegistry({ workspace, database });
    const artifact = await database.memories.rememberArtifact({
      refId: "memref_test",
      toolName: "grep",
      summary: "Large grep output",
      filePath: join(tempDir, "artifact.md"),
      byteLength: 12,
      contentHash: "hash",
    });
    await writeFile(artifact.filePath, "full artifact body", "utf8");

    const result = await registry.execute("read_memory_artifact", {
      refId: "memref_test",
    });
  ...
  it("records read-only explore subagent runs", async () => {
    ...
    await expect(
      database.subagents.getRun(output.runId),
    ).resolves.toMatchObject({
      parentSessionId: "s1",
      agentName: "explore",
      status: "completed",
  ...
  it("lists and reads configured skills", async () => {
    ...
    await writeFile(
      join(skillsRoot, "frontend", "SKILL.md"),
      "# Frontend Builder\n\nUse for UI work.",
    );
    const registry = buildDefaultToolRegistry({
      workspace,
    ...
    const listResult = await registry.execute("list_skills", {
      query: "front",
    });
    const readResult = await registry.execute("read_skill", {
      name: "frontend",
    });
