...
function webSearchDefinition(): ToolDefinition {
  return {
    name: "web_search",
    description: "Search the public web through Brave Search when BRAVE_SEARCH_API_KEY is configured.",
    parameters: {
      type: "object",
      properties: {
        query: { type: "string" },
        count: { type: "integer" },
        country: { type: "string", description: "Optional Brave country code such as US, CN, or GB." }
      },
      required: ["query"]
    }
  };
