...
describe("WorkspaceService", () => {
  ...
  it("blocks traversal, internal paths, and large files", async () => {
    ...
    await expect(workspace.readFile("../outside.txt")).rejects.toBeInstanceOf(WorkspacePathError);
    await expect(workspace.readFile(".git/config")).rejects.toBeInstanceOf(WorkspacePathError);
    await expect(workspace.readFile("large.txt")).rejects.toThrow("File is too large");
  });
...
describe("shell service", () => {
  it("runs safe commands without shell interpretation", async () => {
    const result = await runCommand("node -e \"console.log(process.cwd())\"", { cwd: root });
  ...
  it("can execute approved commands once policy has been handled elsewhere", async () => {
    const result = await runApprovedCommand("node -e \"console.log('approved')\"", { cwd: root });
  ...
  it("truncates large output without splitting utf8 text", async () => {
    const result = await runApprovedCommand("node -e \"console.log('你好'.repeat(20))\"", {
      cwd: root,
      maxOutputBytes: 9
    });
