...
export class DeepSeekChatCompletionsClient implements ModelClient {
  ...
  constructor(options: DeepSeekChatCompletionsClientOptions) {
    this.client =
      options.client ??
      // Narrow the OpenAI-compatible SDK client to DeepSeek chat completions.
      (new OpenAI({
    ...
    for await (const rawChunk of stream) {
      ...
      if (
        ...
        yield {
          ...
          responseId: completionId || 'deepseek_chat_completion'
        };
      }
    }
  }
...
export function toDeepSeekChatTools(
...
export function toDeepSeekChatMessages(
...
export function toDeepSeekThinkingOptions(
  reasoningEffort: ReasoningEffort
...
function mergeToolCallChunk(
  ...
  pendingToolCalls.set(index, {
    ...
    argumentsText:
      current.argumentsText + (chunkToolCall.function?.arguments ?? '')
  });
}
