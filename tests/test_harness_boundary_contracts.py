...
def test_readme_quickstart_uses_python_runtime_commands():
    """README examples cover the two-process setup: Python backend + Next.js frontend.

    The Python/FastAPI backend (port 8000) holds the runtime; the Next.js frontend
    (port 3000) reverse-proxies ``/api/*`` to it. README quickstarts must teach both
    the backend runtime commands AND how to start the frontend dev server.
    """
    ...
    for content in [readme, readme_zh]:
        ...
        # Two-process architecture: the Next.js frontend dev server must be documented.
