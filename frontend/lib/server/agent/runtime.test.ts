...
import { LocalFileArtifactStore } from '../storage/artifact-store'
...
/** plan 模式下恶意模型直接请求写文件，验证执行层 allowlist 会拦截。 */
const planMutatingToolEvents: ResponsesEventFactory = () => [
  {
    type: 'response.output_item.done',
    item: {
      type: 'function_call',
      ...
      name: 'write_file',
      arguments: JSON.stringify({ path: 'notes/plan.md', content: 'should not exist' }),
    },
  },
  { type: 'response.completed', response: { id: 'resp_plan_write', status: 'completed' } },
]
...
describe('Python chat 循环（移植 tests/test_runtime.py 的 scripted-model 用例）', () => {
  ...
  it('大工具输出被卸载为 memory artifact 引用（test_..._offloads_large_tool_outputs）', async () => {
    const root = makeWorkspaceDir()
    const isolatedDb = await createDatabase(':memory:', {
      artifactStore: new LocalFileArtifactStore(root),
    })
    ...
    try {
      ...
      const artifact = await isolatedDb.memories.readArtifactContent(output.refId)
  ...
  it('plan 模式在执行层拒绝 mutating 工具调用', async () => {
    const root = makeWorkspaceDir()
    const events = await collect(
      { input: 'make a plan', session_id: 'sess_plan_guard', mode: 'plan' },
      db,
      root,
      {},
      planMutatingToolEvents,
    )
    ...
    expect(events[3].tool_status).toBe('error')
    expect(events[3].tool_output).toBe('Tool not allowed in the current mode: write_file')
    expect(events[4].code).toBe('tool_not_allowed_in_mode')
    expect(existsSync(join(root, 'notes', 'plan.md'))).toBe(false)

    const transcript = await db.sessions.getTranscript('sess_plan_guard')
    expect(transcript.map((m) => m.role)).toEqual(['user'])
  })
  ...
  it('危险工具调用浮现 approval_required 事件并记审计（test_..._emits_approval_required）', async () => {
    ...
    expect(approval.command).toBe('rm -rf output')
    expect(approval.command_hash).toBe(
      'b73dc5e7188632907365608ca3fb04afd5db2ebead473c2c1d00ee8f29b0ebbb',
    )
